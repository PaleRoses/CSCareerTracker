...
-- Cleanup for deprecated admin policies (now handled via createAdminClient)
...
-- NOTE: Admin user management bypasses RLS via createAdminClient() in the
-- application layer. App-level auth checks (isAdminRole) gate access before
-- any database queries run.
