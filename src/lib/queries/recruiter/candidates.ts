...
/**
 * Get candidates for jobs posted by the current recruiter
 */
export async function getCandidates(filters: CandidateFilters = {}): Promise<Candidate[]> {
  try {
    ...
    // Build query for applications to jobs posted by this recruiter
    let query = supabase
      .from('applications')
      .select(`
        application_id,
...
/**
 * Get detailed candidate info by application ID
 */
...
/**
 * Get counts of candidates by stage for recruiter's jobs
 */
export async function getCandidateCountsByStage(jobId?: string): Promise<Record<string, number>> {
  try {
    ...
    // This is a simplified version - in production you'd use a proper aggregate query
    let query = supabase
      ...
      .is('ended_at', null) // Only active stages
